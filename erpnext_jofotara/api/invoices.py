...
def _build_headers(s):
    """بناء هيدر JoFotara مع استخراج كلمات السر الصحيحة + fallback للـ Device."""
...
def _minify_xml(xml_str: str) -> str:
    ...
    s = re.sub(r">\s+<", "><", s)   # اشطب أي مسافات بين >   <
    s = s.replace("\ufeff", "")     # إزالة BOM لو موجود
    return s
...
def generate_ubl_xml(doc) -> str:
    ...
    - يحدد InvoiceTypeCode = 388 (عادي) أو 381 (مرتجع)
    - يضيف attributes القياسية لـ UNCL1001
    """
    ...
    # نوع الفاتورة: 388 عادي، 381 مرتجع
...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
  ...
  <cbc:InvoiceTypeCode listID="UNCL1001" listAgencyID="6" listAgencyName="UN/ECE" listVersionID="D16B">{inv_code}</cbc:InvoiceTypeCode>
...
def on_submit_send(doc, method=None):
    ...
    # عدم إرسال المرتجع (هنتعامل مع 381 تلقائيًا، بس لو حاب تمنع الإرسال فعلًا خلّيها كده)
    ...
    try:
        ...
        # Debug: خزن نسخة الـ XML بعد الـ Minify على نفس الفاتورة عشان نقدر نراجعه لو حصل خطأ
        if doc.meta.has_field("jofotara_xml"):
            try:
                doc.db_set("jofotara_xml", xml_min)
            except Exception:
                pass
...
def handle_submit_response(doc, resp: dict):
    ...
    if qr and doc.meta.has_field("jofotara_qr"):
        # الـ API عادة بيرجع QR Base64 (PNG) — تقدر تطبعه في Print Format
